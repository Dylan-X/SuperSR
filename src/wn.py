...
def data_based_init(model, input):
    ...
    if type(input) is dict:
        ...
        feed_dict = {tf_inp: np_inp for tf_inp,
                     np_inp in zip(model.inputs, input)}
    else:
        feed_dict = {model.inputs[0]: input}

    # add learning phase if required
    if model.uses_learning_phase and keras.backend.learning_phase() not in feed_dict:
        feed_dict.update({keras.backend.learning_phase(): 1})
    ...
    print(len(layer_output_weight_bias))
